...
class _HomeState extends State<Home> with SingleTickerProviderStateMixin {
  ...
  Future<void> _navigateToSection(String sectionName, GlobalKey key) async {
    ...
    // Close (untilt) the sidebar FIRST and wait for the tilt animation to
    // fully settle. Scrolling while the 3D perspective transform is still
    // engaged causes visible skewing/glitching as new content repaints
    // mid-transform, so we never scroll while tilted.
  ...
  Future<void> _scrollToKey(GlobalKey key) async {
    ...
    try {
      ...
      // Position of the target relative to the scrollable's own content,
      // computed directly so we only ever animate once (no bounce).
      final scrollableBox =
          _scrollController.position.context.storageContext
              .findRenderObject() as RenderBox?;
      final targetTop = scrollableBox == null
          ? box.localToGlobal(Offset.zero).dy
          : box.localToGlobal(Offset.zero, ancestor: scrollableBox).dy;

      final target =
          (_scrollController.offset + targetTop - Globals.headerHeight)
